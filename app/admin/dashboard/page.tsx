  ...
  return (
    <ProtectedRoute>
      <div className="flex flex-col h-screen bg-gray-100">
        {/* Navbar */}
        <nav className="bg-slate-800 text-white p-4 shadow-lg flex justify-between">
                          ...
                          <ul>
                            {order.cartItems.map((item, index) => (
                              <li key={`${order._id}-${index}`} className="flex items-center gap-2">
                                ...
                                {item?.image && (
                                  <Image src={urlFor(item.image).url()} width={40} height={40} alt={item?.productName || "Product Image"} />
                                )}
                              </li>
                            ))}
                          </ul>
